...
def produce_json(topic, rows):
    for r in rows:
        ...
        # attach timestamps
        r["event_timestamp"] = time_calculator.calculate_event_timestamp(r)
        r["ingest_timestamp"] = time_calculator.calculate_ingest_timestamp()
        ...
        producer.produce(
            ...
            key=event_id,
            value=json.dumps(r)
            # partition=hash(event_id) % num_partitions
        )
...
async def main():
    ...
    async def task(block_number: int):
        ...
        tx_count = len(value.get("transactions", []))
